		...
		if IsValid(ent) then
			...
			if SERVER and ent:IsPlayer() then
				if self.Primary.FlashTime then
					if self:GetOwner():GetActiveWeapon():GetClass() == "ls_stunbaton" then
						if self:GetOwner():GetActiveWeapon():GetMode() == 2 then
							ent.stuncounter = ent.stuncounter or 0
							ent.stuncounter = ent.stuncounter + 1
							timer.Create("ResetStun" .. ent:SteamID64(), 2.3, 1, function()
								ent.stuncounter = 0
							end)
							if ent.stuncounter >= 5 then
								ent:Freeze(true)
								timer.Create("Stun" .. ent:SteamID64(), 5, 1, function()
									if ent:IsValid() then
										ent:Freeze(false)
									end
								end)
							end
						elseif self:GetOwner():GetActiveWeapon():GetMode() == 3 then
