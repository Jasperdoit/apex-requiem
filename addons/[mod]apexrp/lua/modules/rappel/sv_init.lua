local function RappelFinish(e,playercolor,movetype,weaponcolor,overridepos)
...
local function PlayerRappel(ply)
...
local function RappelUse(player)
		...
		if ( playerweapon ~= "weapon_physgun" or playerweapon ~= "weapon_physcannon" or playerweapon ~= "gmod_tool") then
			if player:OnGround() then
				if PlayerRappel then
					PlayerRappel(player,entity,self)
...
DarkRP.defineChatCommand("rappel", RappelUse, 1.5);
DarkRP.declareChatCommand{
	command = "rappel",
	description = "Rappel from tall buildings",
	delay = 1.5
};

concommand.Add("rg_rappel",RappelUse)
